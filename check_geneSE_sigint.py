...
sigint_file_list = glob.glob('/home/sillo/CRC2019/SE_hijack/cis/output/01_sigint/*.csv.gz')
...
for line in f:
	...
	strand = linedata[3]
	if strand == "+": 
		tss1 = (int(linedata[1])/20000) * 20000
		tss2 = tss1 - 20000
		if tss2 < 0: tss2 = 0
		binid = ".".join([chrname, str(tss2), str(tss1)])

	elif strand == "-":
		tss1 = (int(linedata[2])/20000) * 20000
		tss2 = tss1 + 20000
...
def check_SE_in_cont(bincoord, chrname, se_dict):
	...
	for start, end in selist:
		if start <= bincoord <= end or start <= bincoord + 20000 <= end:
...
outfile_path = '/home/sillo/CRC2019/SE_hijack/cis/output/02_geneSEpair'
